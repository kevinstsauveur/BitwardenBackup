    ...
    for ($num = $list.Length-1; $num -ge 0; $num--){
		...
		if(($currentMonth -ne $tempCurrentMonthNumber) -and ($currentMonth-1 -ne $tempCurrentMonthNumber)){ #Si c'est pas le mois actuel ou le mois dernier, supprimer s'il y a plus d'un fichier par mois
            if($countMonth -gt 1){
                ...
                WriteToLogFile "Delete $filename : Mois #$currentMonthNumber : Compt� $countMonth fois"
            }
        }Elseif(($currentWeek -ne $tempCurrentWeekNumber) -and ($currentWeek-1 -ne $tempCurrentWeekNumber)){ #Si c'est pas la semaine actuelle ou la semaine pass�
